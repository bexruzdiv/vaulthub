...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            disable_local_ca_jwt=dict(type='str', required=True),
            skip_fail=dict(type='bool', required=False, default=False)
        )
    )
    ...
    disable_local_ca_jwt = module.params['disable_local_ca_jwt']
    skip = module.params['skip_fail']

    # Check if the configuration already exists
    existing_config = check_existing_config(module, k8s_auth_name, vault_address, vault_token)
    if existing_config:
        if skip:
            module.exit_json(changed=False, msg=f"Vault authentication configuration for '{k8s_auth_name}' already exists")
        else:
    ...
    command += f"disable_local_ca_jwt='{disable_local_ca_jwt}'"
    # Execute the command
    result = subprocess.run(
        command,
        shell=True,
